...
class SmsDemo
{
     ...
     */
    public static function sendMsgToDrive($mobile,$start,$end)
    {
        $sign=SysConfig::where('key','yzm_sign')->value('value');
        if(!$sign){
            $msg="$mobile 发送验证码失败，原因是：验证码签名错误";
        //    Log::channel('msgLog')->write($msg);
            throw new OrderException(['msg'=>'验证码签名错误']);

        }
        $temp=SysConfig::get('yzm_driver');
        // 初始化SendSmsRequest实例用于设置发送短信的参数
        $request = new SendSmsRequest();
        ...
        if(!isset($res['Code'])){
            Log::error("失败");
            $msg="$mobile 发送验证码失败，原因是:$acsResponse";
          //  Log::channel('msgLog')->write($msg);
            throw new BaseException(['msg'=>'验证码发送失败']);
        }else{
            Log::error("成功");
            if($res['Code']!='OK'){
                $msg="$mobile 发送短信失败，原因是:".$res['Message'];
               // Log::channel('msgLog')->write($msg);
     ...
     * @throws OrderException
     */
    public static function sendMsgToReciver($mobile,$order_num,$car_num,$name,$drvier_mobile)
    {
        $sign=SysConfig::where('key','yzm_sign')->value('value');
        if(!$sign){
            $msg="$mobile 发送验证码失败，原因是：验证码签名错误";
            Log::error($msg);
          //  Log::channel('msgLog')->write($msg);
            throw new OrderException(['msg'=>$msg]);

        }
        $temp=SysConfig::get('yzm_reciver');
        // 初始化SendSmsRequest实例用于设置发送短信的参数
        $request = new SendSmsRequest();
        ...
        //    Log::channel('msgLog')->write($msg);
            ...
            Log::error("成功");
            if($res['Code']!='OK'){
                $msg="$mobile 发送验证码失败，原因是:".$res['Message'];
                Log::error($msg);
              //  Log::channel('msgLog')->write($msg);
                throw new BaseException(['msg'=>$msg]);
            }
